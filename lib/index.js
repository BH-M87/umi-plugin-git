...
function _ref() {
  _ref = _asyncToGenerator(function* (api) {
    ...
    try {
      const now = Date();
      api.logger.info('Current build time: ', now);
      ...
      }); // 100MB, use default 200KB will throw error, 10000 commit's size is around 4MB
      ...
      api.modifyHTML($ => {
        ...
        $('head').prepend(`<meta name="Current build time" content="${now}"></meta>`);
        return $;
      });
      api.logger.info('Use git plugin to modify html success.');
    } catch (error) {
      api.logger.error('Use git plugin failed: ', error);
    }
  });
  return _ref.apply(this, arguments);
}